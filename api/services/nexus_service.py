...
class NexusService(BaseIntegradorService):
    tenant = TenantEnum.NEXUS
    ...
    def process_file(self, path: Path):
        ...
        try:
            from core.nexus_ingester import NexusDataIngester

            ingester = NexusDataIngester(
                tenant="NEXUS",
            ...
            for converted_path in ingester.convert_csv(path):
                ...
                if df_upload.empty:
                    logger.warning(f"[NEXUS] No healthy rows in {converted_path.name}")
                    continue

                df_upload.to_csv(converted_path, index=False)
                ok = ingester.process_converted_with_api_parallel(converted_path)
                if ok:
                    parts_uploaded += 1
                else:
            ...
            logger.error(f"[NEXUS] core/nexus_ingester.py not found: {e}")
            errors.append(self._error(path.name, str(e), ValidationStage.PARSE))
        except Exception as e:
            logger.error(f"[NEXUS] Unexpected error: {e}", exc_info=True)
